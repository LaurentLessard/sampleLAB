...
amplitude = [1.4 0.9 1.1 0.8];
window_size = 30;
noise_strength = 2;
fp = plot_fig(max_muR, min_muR, alphaR, betaR, amplitude, window_size, noise_strength,'autumn');

function Fp = plot_fig(max_muR, min_muR, alphaR, betaR, amplitude, window_size, noise_strength, cmap)
    ...
    % amplitude     : 1x4 vector with amplitude values for each distribution
    % window_size   : scalar
    ...
    for i = 1:n
        ...
        F = (amplitude(1)*f1) + (amplitude(2)*f2) + (amplitude(3)*f3) + (amplitude(4)*f4);
